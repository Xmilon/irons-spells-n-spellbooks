...
public class ScrollForgeMenu extends AbstractContainerMenu {
    ...
    public ScrollForgeMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, resolveBlockEntity(inv, extraData));
    }
    ...
    public ScrollForgeMenu(int containerId, Inventory inv, BlockEntity entity) {
        ...
        blockEntity = entity instanceof ScrollForgeTile tile ? tile : createClientFallbackTile(inv.player.blockPosition());
    ...
    private static BlockEntity resolveBlockEntity(Inventory inv, FriendlyByteBuf extraData) {
        BlockEntity entity = null;
        if (extraData.readableBytes() >= Long.BYTES) {
            entity = inv.player.level().getBlockEntity(extraData.readBlockPos());
        }
        if (entity instanceof ScrollForgeTile) {
            return entity;
        }
        BlockPos center = inv.player.blockPosition();
        for (int x = -6; x <= 6; x++) {
            for (int y = -3; y <= 3; y++) {
                for (int z = -6; z <= 6; z++) {
                    BlockPos pos = center.offset(x, y, z);
                    BlockEntity nearby = inv.player.level().getBlockEntity(pos);
                    if (nearby instanceof ScrollForgeTile) {
                        return nearby;
                    }
                }
            }
        }
        return createClientFallbackTile(center);
    }

    private static ScrollForgeTile createClientFallbackTile(BlockPos pos) {
        return new ScrollForgeTile(pos, SCROLL_FORGE_BLOCK.get().defaultBlockState());
    }
